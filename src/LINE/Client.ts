 ...
 * gitHub: https://github.com/kobanyan/line-bot-sdk-gas
 * @author kobanyan
 */
...
class LineClient {
  ...
  public getProfileWithEventSource(
    eventSource: Line.EventSource,
  ): Line.Profile {
  ...
  private messagingApiUrl = (path: string): string =>
    `${LineClient.messagingUrl}${path}`;
  private dataApiUrl = (path: string): string => `${LineClient.dataUrl}${path}`;
  ...
  private contentUrl = (messageId: string) =>
    this.dataApiUrl(`message/${messageId}/content`);
  private userProfileUrl = (userId: string) =>
    this.messagingApiUrl(`profile/${userId}`);
  ...
  private profileUrl = (eventSource: Line.EventSource) => {
    switch (eventSource.type) {
      case 'group':
        return this.groupMemberProfileUrl(
          eventSource.groupId,
          eventSource.userId,
        );
      case 'room':
        return this.roomMemberProfileUrl(
          eventSource.roomId,
          eventSource.userId,
        );
      default:
  ...
  private roomMemberIdsUrl = (roomId: string) =>
    this.messagingApiUrl(`room/${roomId}/members/ids`);
  private leaveGroupUrl = (groupId: string) =>
    this.messagingApiUrl(`group/${groupId}/leave`);
  private leaveRoomUrl = (roomId: string) =>
    this.messagingApiUrl(`room/${roomId}/leave`);
  ...
  private userRichMenuUrl = (userId: string, richMenuId?: string) =>
    this.messagingApiUrl(
      `user/${userId}/richmenu${richMenuId ? `/${richMenuId}` : ''}`,
    );
  ...
  private defaultRichMenuUrl = (richMenuId?: string) =>
    this.messagingApiUrl(
      `user/all/richmenu${richMenuId ? `/${richMenuId}` : ''}`,
    );
